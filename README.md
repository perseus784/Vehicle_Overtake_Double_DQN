...
**Idea:** The main idea is to teach the RL algorithm to how to do an overtaking action in a top-down view highway environment.
...
Our goal is to help the user car overtake the bot cars on its own in a roadway environment. We train the user car with the help of deep reinforcement learning[3], the reward function will penalise the user car every time it slows down, every time it crashes into bot car and if there are any bot cars in front of it. The reward function will reward the user car whenever it overtakes a bot car. Since we use the raw pixels from the environment as our state space, we can easily easily change the environment and the reward function to adapt to a new problem.
 ...
 \begin{figure}[htp]
    \centering
    \includegraphics[width=8cm]{figures/InkedInside network_LI.jpg}
    \caption{Raw pixel Feed}
\end{figure}
...
The tools we use in this project are OpenAI gym like Highway Environment, Tensorflow and Python. Highway Environment which gives us a environment for our model to train, it is a collection of environments for autonomous driving and tactical decision-making tasks. We use Tensorflow to develop and train Reinforcement Learning. We use Python as a programming Language for this project.

\begin{itemize}
    \item Operating System: Linux, Windows 10 or MacOS
    \item RAM: 8GB
    \item Graphics Card VRAM: 2GB Nvidia GT970M
    \item SSD Storage: 256GB
\end{itemize}

We have all these hardware minimum requirements meet to run the project with the suitable environment.We need to install three tools. The tools we need are Highway Environment, Python and Tensorflow.

\subsection*{Command to install}
\begin{itemize}
...
\textbf{Pseudo Code}
\begin{algorithmic}[1]
\STATE Init Predicting Network
\STATE Init Training Network
\STATE epsilon = 1
\STATE $memory\gets \{\}$
\FORALL {$episodes$}
    \STATE obs = Start Environment Render
    \FORALL {$duration$}
        \STATE action = Training Network(obs)
        \STATE do action  \{get next observation\}
        \STATE reward = get-reward(action)
        \STATE $memory \gets (obs,action,reward,next obs)$\\
        \IF {$step/4$}
            \STATE train the Training network(memory)
        \ENDIF
        \IF {$step< total steps//2$}
            \STATE Reduce $\epsilon$ by decay
        \ENDIF
    \ENDFOR
\IF {$episodes/10$}
    \STATE Update Predicting Network with new weights
\ENDIF
\ENDFOR
\end{algorithmic}  
...
\begin{figure}[H]
    \centering
    \includegraphics[width=8.5cm]{figures/Double DQN.png}
    \caption{Training and predicting networks in action}
    \label{fig:trpr}
\end{figure}
...
As the agent learns and traverses through the episodes, it forgets the previously learned experiences. So, when we train it is important to recursively remind the network of what it learnt and the way to do that is is using a experience replay mechanism. This mechanism helps the network remember it's experience by randomly picking from these while training.
 ...
 \begin{figure}[htp]
    \centering
    \includegraphics[width=8cm]{figures/ex_replay.png}
    \caption{Experience replay mechanism}
\end{figure}
...
\subsection{Exploration Vs Exploitation}
 ...
 \begin{figure}[H]
    \centering
    \includegraphics[width=8cm, height=6cm]{figures/image7.png}
    \caption{$\epsilon$ reducing over 1500 steps}
    \label{fig:epsilon}
\end{figure}
...
\begin{itemize}
    \item $\epsilon$ (Exploration vs Exploitation) value was tried with different decay values and over a different period of time like not decaying till the first half the training and then decay over last half, decay within the first third of the episodes, no exploration etc. But what worked for us was slowly reducing the epsilon over the first half of the training.
    \item Learning Rate on the other hand we went with the default tensorflow Adam learning Rate but it was causing us more fluctuations in the loss during the training so went with a reduced 1e-4 learning rate for our training network. This helped reducing the fluctuating loss.
    \item We used a batch size of 32 for the final training. Initially, we went with the higher batch size such as 256, 512 but those did not show improvements and also it occasionally caused memory exhaust errors. 
    ...
    \item The other factor is how often we update the online or predicting network for transferring the experience. This was done for every 10 episodes so that the network predictions was made better over time and also not too much of updating the online network to cause too much instability in the predictions.
\end{itemize}
...
The loss graph will show that the network is learning better after each epoch as it shows the overall the network is making better decisions. The loss graph is drawn for the training network. The loss graph is much smoother after we reduced the learning rate to a very small value and we have also smoothend the loss value using Tensorboard. The following graph is what we got for the final iteration of our implementation. 
 \begin{figure}[H]
    \centering
    \includegraphics[width=8cm, height=6cm]{figures/image2.png}
    \caption{Loss graph of our network}
    \label{fig:Deep}
\end{figure}

\subsubsection{Reward Graph}
The rewards graph will show that as the epochs keep increasing the rewards will also increase because as the network learns more, it will perform actions which gives it the maximum reward. The original paper where they developed the Double DQN technique, the authors ran the environment for 250M epochs compared to us, we only ran the training for 250K times.
 ...
 \begin{figure}[H]
    \centering
    \includegraphics[width=8cm, height=6cm]{figures/image5.png}
    \caption{Reward graph of our network}
    \label{fig:Deep}
\end{figure}
